...
Currently configured project: `jde-client` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/jde-client/SETUP.md` to fill in
   `data/jde-client/INFO.md`.
...
`--project-id jde-client` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/jde-client/`.
