# Agent setup for `jde-client`

This is a deepsec scanning workspace. Project `jde-client` was just registered
(target: `..`). Setup is incomplete — `data/jde-client/INFO.md`
...
2. **Fill in `data/jde-client/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id jde-client
pnpm deepsec process --project-id jde-client
```
