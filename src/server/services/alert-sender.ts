...
// Nombre max de lignes chargées en mémoire par run (borne la RAM).
// Les jobs restants sont traités au run suivant.
...
export async function sendPendingAlertJobs(options: { dryRun?: boolean; verbose?: boolean } = {}): Promise<AlertSenderResult> {
  ...
  // Boucle par pages : charge PAGE_SIZE jobs à la fois pour borner la RAM,
  // et continue jusqu'à épuisement de la file (au sein d'un seul run CLI).
  while (true) {
    const pendingJobs = await db
      ...
      .where(and(selection, sql`${alertJobs.id} > ${lastId}`))
      .orderBy(alertJobs.id)
      .limit(PAGE_SIZE)

    if (pendingJobs.length === 0) break

    lastId = pendingJobs[pendingJobs.length - 1]!.jobId
    ...
    if (pendingJobs.length < PAGE_SIZE) break
  }
